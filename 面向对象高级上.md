...
- header中的防卫式声明
  - ![1615894277820](asserts\1615894277820.png)

- header的布局
  - ![1615894480379](asserts\1615894480379.png)
...
- constructor(ctor，构造函数)
  ...
  - ![1615896924226](asserts\1615896924226.png)
  - 而不是在大括号里再赋值
...
- 函数后加const![1615897994138](asserts\1615897994138.png)
  - 常量成员函数
...
- temp object临时对象       通过typename()来创建，生命到下一行就结束了
  - ![1615959025300](asserts\1615959025300.png)
...
- ctor和dtor(构造函数和析构函数)
  - ![1615977115455](asserts\1615977115455.png)
...
- static local objects，离开scope，生命依然存在，直到程序结束
  - ![1615980214813](asserts\1615980214813.png)
- global objects，任何大括号之外的，生命整个程序
- new的分解步骤：
  1. 先malloc分配sizeof(对象)的内存
  2. 把void*的内存空间转型成对应的类型
  3. 用得到的类型正确的指针调用构造函数
  4. ![1615980762220](asserts\1615980762220.png)
- delete的分解步骤：
  1. 先调用dtor析构函数
  2. 再释放内存
  3. ![1615980851171](asserts\1615980851171.png)
- 动态分配所得的内存块，在vc编译器下
  - 可以看到，debug模式下和release模式下，内存占用大不相同
  - ![1615981667052](asserts\1615981667052.png)
- array new要搭配array delete，不然会带来内存泄漏
  - 动态分配array时：
  - ![1615982320140](asserts\1615982320140.png)
  - 如果没有搭配array delete：内存泄露的是没有被dtor的，剩余的几个
  - ![1615982661020](asserts\1615982661020.png)
...
- 补充static
  - 非静态时的调用，实质是这样的
  - ![1616046535678](asserts\1616046535678.png)
  - 静态的数据和代码在内存中都只有一份。
  - 静态数据要在类外定义，给不给初值都可以
  ...
  - 调用静态函数有两种方法：
    ...
    - 也可以通过class name来调用
- 补充，把ctors放在private区
  - 单例模式(饿汉式)：![1616047130590](asserts\1616047130590.png)
  - 懒汉单例：![1616047301453](asserts\1616047301453.png)
- 补充，cout
...
- 更多细节与深入：![1616048458641](asserts\1616048458641.png)

# P11 组合与继承

- Composition复合，表示has-a

  - ![1616050805846](asserts\1616050805846.png)
...
- composition关系下的构造和析构
  ...
  - 构造函数由内而外，先调用内部组件的default构造函数，再构造自己；
    ...
    ![1616051479340](asserts\1616051479340.png)

  - 析构函数由外而内，先执行自己的，再析构内部组件的析构![1616051570549](asserts\1616051570549.png)
...
- Inheritance继承，表示is-a
  ...
  - ![1616052545708](asserts\1616052545708.png)
...
- Inheritance with virtual functions虚函数
  ...
  - ![1616059070313](asserts\1616059070313.png)
- 设计模式，模板方法(template method)
  - ![1616059944513](asserts\1616059944513.png)
  - 常用在框架里。把固定能写好的先写好，剩下不确定的写成虚函数，留给子类去实现
- Inheritance+Composition关系下的构造和析构
- Delegation+Inheritance
  - ![1616061177424](asserts\1616061177424.png)
  - 观察者模式
...
- composite，[组合模式](https://blog.csdn.net/lory17/article/details/62940496)
  - ![1616071676981](asserts\1616071676981.png)
- prototype
  ...
  - ![1616072671206](asserts\1616072671206.png)
